...
export async function GET(_req: Request, { params }: { params: { token: string } }) {
  ...
  markInvitationOpened(invitation.id);

  const questions = listQuestions(event.id);
  const latest = getLatestResponse(invitation.id);
  const answers = latest ? getAnswersForResponse(latest.id) : [];
  const members = invitation.invitee?.group_id ? groupMembers(invitation.invitee.group_id) : [];
